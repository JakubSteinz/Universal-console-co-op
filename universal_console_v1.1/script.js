...
class ConsoleApp {
    ...
    init() {
        ...
        // Event Listeners
        this.cmdInput.addEventListener('input', (e) => this.handleInput(e));
        this.cmdInput.addEventListener('keydown', (e) => {
            if (this.suggestionPopup.classList.contains('hidden')) {
                // Normal behavior
                if (e.key === 'Tab') {
                    e.preventDefault();
    ...
    getAllCommands() {
        const cmds = new Set();
        // Global
        Object.values(this.commandCategories).flat().forEach(c => {
            cmds.add(c.split(' ')[0]);
        });
        // World
        if (this.activeWorld) {
            Object.keys(this.activeWorld.commands).forEach(c => cmds.add(c));
        }
    ...
    handleInput(e) {
        ...
        if (val.toLowerCase().startsWith('theme')) {
            ...
            if (parts.length > 0 && parts[0].toLowerCase() === 'theme') {
                // Show popup
                this.showThemePopup(parts[1] || '');
            } else {
                this.closePopup();
            }
        } else {
        ...
        const suggestions = this.getAllCommands().filter(c => c.startsWith(val.toLowerCase()));
        if (suggestions.length > 0) {
            const best = suggestions[0];
            const remainder = best.substring(val.length);
            this.ghostInput.value = val + remainder;
        }
    }

    // Popup Methods
    showThemePopup(filter) {
        const themes = ['amber', 'retro', 'cyber', 'ice', 'crimson', 'void', 'royal'];
        const matches = themes.filter(t => t.startsWith(filter.toLowerCase()));
        ...
        matches.forEach((theme, idx) => {
            ...
            this.popupItems.push(theme);
        });

        this.suggestionPopup.classList.remove('hidden');
    }

    closePopup() {
        this.suggestionPopup.classList.add('hidden');
        this.popupItems = [];
    }

    navigatePopup(dir) {
        if (!this.popupItems || this.popupItems.length === 0) return;

        const items = this.suggestionPopup.querySelectorAll('.suggestion-item');
        items[this.popupIndex].classList.remove('selected');

        this.popupIndex = (this.popupIndex + dir + this.popupItems.length) % this.popupItems.length;

        items[this.popupIndex].classList.add('selected');
        items[this.popupIndex].scrollIntoView({ block: 'nearest' });
    }

    selectPopupItem() {
        if (!this.popupItems || this.popupItems.length === 0) return;
        this.applyThemeSelection(this.popupItems[this.popupIndex]);
    }

    applyThemeSelection(theme) {
        this.cmdInput.value = `theme ${theme}`;
        this.ghostInput.value = '';
        this.closePopup();
        this.cmdInput.focus();
        // Optional: Preview functionality? Not requested, sticking to selection.
    }

    handleTabComplete() {
        if (this.ghostInput.value) {
            this.cmdInput.value = this.ghostInput.value;
            this.ghostInput.value = '';
        }
    }
    ...
    processCommand(rawCmd) {
        ...
        switch (mainCmd) {
            case 'help':
                ...
                const cats = this.commandCategories;
